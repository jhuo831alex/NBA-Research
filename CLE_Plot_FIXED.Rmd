...
###
answeet<-allplayersweet %>% group_by(converted_x,converted_y) %>% summarise(n=n())
annonsweet<-allplayernonsweet %>% group_by(converted_x,converted_y) %>% summarise(n=n())
## 821 sweet points and 950 non-sweet points
821/(821+950) ## 0.464
821+950 ## 1771
...
g4<-ggplot()+geom_point(data=allplayersweet,aes(x=converted_x,y=converted_y,color=PCTG,size=Shots,shape=Position))+geom_point(data=allplayernonsweet,aes(x=converted_x,y=converted_y,color=PCTG,size=Shots,shape=Position))+scale_color_gradientn(colours=c("black","grey","white","orange","red"),values=c(0,0.40,0.45,0.50,1),breaks=c(0.4,0.5))+
  ...
  coord_fixed()+ggtitle("Sweet and Unsweet Spots for Cleveland Cavaliers (1 sqft)")+scale_size_continuous(limits=c(0,70),breaks=c(10,25,70))+theme_void(base_size =9) +
